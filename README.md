...
1.5.0 将新闻与社交平台动态合并到主页双 Tab，并提供一次完成“按勾选抓取、中文整理及写入”的工作流；Google 凭据支持 Windows 原生文件选择，翻译支持 Gemini、Groq、OpenAI 多平台及多密钥轮询/故障切换。1.4.1 已删除未使用且存在已知漏洞的 `python-multipart` 依赖，并在 Windows 打包配置中明确排除其模块。
